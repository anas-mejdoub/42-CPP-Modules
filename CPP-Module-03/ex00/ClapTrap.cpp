...
/*   Updated: 2024/11/09 17:37:40 by amejdoub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    std::cout << "ClapTrap default constr called" << std::endl;
    _name = name;
    _hitPoint = 10;
    _energyPoint = 10;
    _attackDamage = 0;
...
void ClapTrap::beRepaired(unsigned int amount)
{
    if (_hitPoint != 0 && _energyPoint != 0)
    {
        std::cout << "ClapTrap " << _name << " repairs itself "  << amount << " hit point now has " << _hitPoint + amount << std::endl;
        _energyPoint--;
        _hitPoint += amount;
    }
    else
...
{
    std::cout << "ClapTrap " << _name << " took " << amount << " amount of dammage " << std::endl;
    _hitPoint -= amount;
    if (_hitPoint < 0)
        _hitPoint = 0;
}